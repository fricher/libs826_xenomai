...
static unsigned int eeprom_fault[S826_MAX_BOARDS];      // Detected eeprom faults (missing/faulty device) for each board index.
...
#define CAL_FIXPT_1R0       ((unsigned int)0x80000000)          // Default fixed-point, unsigned calibration scalar as stored in eeprom (0x80000000 = +1.0).
...
static void SpiWaitIdle_EEL(unsigned int index)
{
...
static unsigned int SpiTransact_EEL(unsigned int index, unsigned int bitcnt, unsigned int txval)
{
    unsigned int rxval;
...
static void EepromCsAssert_EEL(unsigned int index)
{
...
static void EepromCsNegate_EEL(unsigned int index)
{
...
static unsigned int EepromCmd_EEL(unsigned int index, unsigned int bitcnt, unsigned int cmd)
{
    unsigned int rxval;
...
static unsigned int EepromCmdREAD_EEL(unsigned int index, unsigned int addr)
{
    return EepromCmd_EEL(index, 24, EECMD_READ | EEADR(addr));
}

static unsigned int EepromCmdRDSR_EEL(unsigned int index)
{
    return EepromCmd_EEL(index, 16, EECMD_RDSR);
}

static void EepromCmdWRITE_byte_EEL(unsigned int index, unsigned int addr, unsigned int data)
{
    EepromCmd_EEL(index, 24, EECMD_WRITE | EEADR(addr) | data);
}

static void EepromCmdWRITE_quadlet_EEL(unsigned int index, unsigned int addr, unsigned int data)
{
...
static void EepromCmdWREN_EEL(unsigned int index)
{
    EepromCmd_EEL(index, 8, EECMD_WREN);
}
#if 0
static void EepromCmdWRDI_EEL(unsigned int index)
{
...
static int EepromDetect_EEL(unsigned int index)
{
...
static void EepromByteWrite_EEL(unsigned int index, unsigned int addr, unsigned int data)
{
...
static void EepromQuadletWrite_EEL(unsigned int index, unsigned int addr, unsigned int data)
{
    EepromCmdWREN_EEL(index);                                   //# enable eeprom writes
...
static unsigned int EepromQuadletRead_EEL(unsigned int index, unsigned int addr)
{
    int i;
    unsigned int data = 0;
...
S826_API int S826_CC S826_EepromWriteByte(unsigned int board, unsigned int addr, unsigned int data)
{
    ...
    if ((index = bdindex[board]) == (unsigned int)VACANT_BOARDINDEX)            // Map board to index number, error if board is closed.
...
static unsigned int DacCmdSetRange(unsigned int chan, unsigned int range)
{
...
static unsigned int DacCmdSetOutput(unsigned int chan, unsigned int data18)
{
    return  ((chan & 4) << 22)                                              // device select
...
static unsigned int DacUserToRaw(unsigned int index, unsigned int setpoint, unsigned int range)
{
    ...
    return (range & DAC_RANGE_BIPOLAR) ?                                                            // convert user setpoint to raw:
        (unsigned int)(int)((int16_t)(setpoint - DAC_BIPOLAR_ZERO) * cal + DAC_BIPOLAR_ZERO_FIXEDPOINT) : //   if bipolar range
        (unsigned int)(setpoint * cal);                                                                   //   if unipolar range
}
...
static unsigned int DacRawToUser(unsigned int index, unsigned int raw, unsigned int range)
{
    double cal = DacRangeCal[index][range] * DAC_GUARDBIT_SCALAR;       // get cal scalar for specified output range

    unsigned int raw18 = ((raw & DAC_MASK_SETPOINT) << DAC_NUM_GUARDBITS)       // build fixed-point raw dac data by combining integer part
    ...
    return (range & DAC_RANGE_BIPOLAR) ?                                                        // convert raw setpoint to calibration-corrected:
        (unsigned int)(((int)raw18 - DAC_BIPOLAR_ZERO_FIXEDPOINT) / cal + DAC_BIPOLAR_ZERO + 0.5) :     //   if bipolar range
        (unsigned int)(raw18 / cal + 0.5);                                                              //   if unipolar range
}
...
static void DacRangeWrite(unsigned int index, unsigned int baseaddr, unsigned int chan, unsigned int range)
{
    static const unsigned int DacZero[] = {0, 0, DAC_BIPOLAR_ZERO_FIXEDPOINT, DAC_BIPOLAR_ZERO_FIXEDPOINT};     // 0V setpoint values for each DAC output range.
...
static void DacDataWrite(unsigned int index, unsigned int baseaddr, unsigned int chan, unsigned int setpoint)
{
    unsigned int range;
    unsigned int cmd;
...
static int DacRead(unsigned int index, unsigned int baseaddr, unsigned int chan, unsigned int *range, unsigned int *setpoint, unsigned int safemode)
{
    unsigned int span;
    unsigned int data;
...
S826_API int S826_CC S826_DacRead(unsigned int board, unsigned int chan, unsigned int *range, unsigned int *setpoint, unsigned int safemode)
{
    ...
    if (board >= S826_MAX_BOARDS)                                                       // Abort if invalid board number.
        return S826_ERR_BOARD;
    if ((index = bdindex[board]) == (unsigned int)VACANT_BOARDINDEX)                            // Map board to index number; abort if board closed.
        return S826_ERR_BOARDCLOSED;
...
static int DacCalEepromRead_EEL(unsigned int index)
{
    int range;
...
S826_API int S826_CC S826_DacCalRead(unsigned int board, double slope[4], unsigned int *valid)
{
    unsigned int range;
    unsigned int index;
    int errcode;
    ...
    if ((errcode = sys826_lock(index, MUTEX_EEPROM)) == S826_ERR_OK)            //# Lock eeprom.
    {
        for (range = 0; range < DAC_NUM_RANGES; range++)
        {
            ...
            EepromQuadletWrite_EEL(index, S826_EEADR_DACDATA + 4 * range,       //# Copy new value to eeprom.
                (unsigned int)(scalars[range] * (double)CAL_FIXPT_1R0));
        }
...
static void AdcCalActivateEx_EEL(unsigned int index, unsigned int baseaddr)
{
    int slot;
    int range;
...
// # EEPROM LOCK MUST BE SET BEFORE CALLING THIS FUNCTION.
static int AdcCalActivate_EEL(unsigned int index)
{
    int errcode;
...
static int AdcCalEepromRead_EEL(unsigned int index)
{
    int range;
    ...
    if ((errcode = sys826_lock(index, MUTEX_EEPROM)) == S826_ERR_OK)                    //# Lock board so no other processes can read cal values while we write new ones.
    {
        for (range = 0; range < ADC_NUM_RANGES; range++)
        {
            ...
            EepromQuadletWrite_EEL(index, S826_EEADR_ADCDATA + 8 * range + 4,
                (unsigned int)(*slope++ * (double)CAL_FIXPT_1R0));
        }
...
static unsigned int BuildAdcConfig(unsigned int chan, unsigned int tsettle, unsigned int gain)
{
    unsigned int cfg = ((chan & 0x1F) << 2) | gain;                                             // External/internal mux, analog channel, bipolar span (0=10V, 1=5V, 2=2V, 3=1V).
    if (tsettle > 0)
...
S826_API int S826_CC S826_AdcSlotConfigRead(unsigned int board, unsigned int slot, unsigned int *chan, unsigned int *tsettle, unsigned int *gain)
{
    unsigned int index;
    unsigned int regval = 0;       // default return value upon error
    int errcode;
    if (board >= S826_MAX_BOARDS)                                                       // Abort if invalid board number.
        return S826_ERR_BOARD;
    if ((index = bdindex[board]) == (unsigned int)VACANT_BOARDINDEX)                            // Map board to index number; abort if board closed.
...
static void AdcTrigModeWrite(unsigned int index, unsigned int router_baseaddr, unsigned int trigmode)
{
...
static int AdcWaitDav(unsigned int index, unsigned int *slotlist, unsigned int tmax)
{
#ifndef RT_SUPPORT
    unsigned int tstart;
...
//    int soi = *slotlist;        // Slots Of Interest.
    ...
    if ((unsigned int) ready_soi == *slotlist)                                             // Done if all slots of interest have been converted
        return S826_ERR_OK;
    ...
    while (1)                                                               // Repeat forever:
    {
        ...
        case S826_ERR_NOTREADY:                                                 // WAIT TIMED OUT
            ...
            if (telapsed >= tmax)                                               // If an actual timeout ...
            {
                ...
                if ((unsigned int) ready_soi == *slotlist)                                     //   Check in case all soi's completed after the wait, and if so,
...
static int AdcDataCopy(unsigned int index, unsigned int slotlist, int *data, unsigned int *timestamps, int calcorrect)
{
    int         errcode;
    int         slot;
    unsigned int        baseaddr_data;
    unsigned int        baseaddr_tstamp;
...
static int AdcRead(unsigned int index, int *data, unsigned int *timestamps, unsigned int *slotlist, int calcorrect, unsigned int tmax)
{
    int errcode;
...
S826_API int S826_CC S826_AdcRead(unsigned int board, int *data, unsigned int *timestamps, unsigned int *slotlist, unsigned int tmax)
{
    unsigned int index;
    if (board >= S826_MAX_BOARDS) {                                                     // Abort if invalid board number.
        *slotlist = 0;
...
S826_API int S826_CC S826_AdcRawRead(unsigned int board, int *data, unsigned int *timestamps, unsigned int *slotlist, unsigned int tmax)
{
    unsigned int index;
    if (board >= S826_MAX_BOARDS) {                                                     // Abort if invalid board number.
        *slotlist = 0;
...
S826_API int S826_CC S826_DioCapEnablesRead(unsigned int board, unsigned int *rising, unsigned int *falling)
{
    unsigned int index;
    if (board >= S826_MAX_BOARDS)                                                   // Abort if invalid board number.
        return S826_ERR_BOARD;
...
// Helper functions.
static void WatchdogConfigRead(unsigned int index, unsigned int baseaddr, unsigned int *config, unsigned int timers[5])
{
...
static void WatchdogConfigWrite(unsigned int index, unsigned int baseaddr, unsigned int config, unsigned int timers[5])
{
...
{
    unsigned int index;
    unsigned int wval = S826_WDCFG_EN | (enable ? PORT_BITSET : PORT_BITCLR);
    int errcode;
    if (board >= S826_MAX_BOARDS)                                               // Abort if invalid board number.
        return S826_ERR_BOARD;
    if ((index = bdindex[board]) == (unsigned int)VACANT_BOARDINDEX)                    // Map board to index number; abort if board closed.
...
static int CounterRegWrite(unsigned int board, unsigned int chan, unsigned int regos, unsigned int regval)
{
    unsigned int index;
    int errcode;
    if (board >= S826_MAX_BOARDS)                                       // Abort if invalid board number.
...
static int CounterRegRead(unsigned int board, unsigned int chan, unsigned int regos, unsigned int *regval)
{
    unsigned int index;
    int errcode;
    if (board >= S826_MAX_BOARDS)                                       // Abort if invalid board number.
...
static int ReadSnapshot(unsigned int index, unsigned int baseaddr, unsigned int *counts, unsigned int *tstamp, unsigned int *reason)
{
    unsigned int status;
...
// API function.
S826_API int S826_CC S826_CounterSnapshotRead(unsigned int board, unsigned int chan, unsigned int *counts, unsigned int *tstamp, unsigned int *reason, unsigned int tmax)
{
    unsigned int index;
    int errcode;
    if (board >= S826_MAX_BOARDS)                                                           // Abort if invalid board number.
    ...
    if (flataddr[index])                                                                    // FLAT:
    {
        unsigned int baseaddr = S826_ADR_CTR(chan);                                                 // Cache counter registers base address
...
S826_API int S826_CC S826_CounterSnapshotConfigWrite(unsigned int board, unsigned int chan, unsigned int cfg, unsigned int mode)
{
    return CounterRegWrite(board, chan, S826_OS_SSCTL, (mode << 30) | (cfg & 0x3FFFFFFF));
}

S826_API int S826_CC S826_CounterSnapshotConfigRead(unsigned int board, unsigned int chan, unsigned int *cfg)
{
...
S826_API int S826_CC S826_CounterExtInRoutingWrite(unsigned int board, unsigned int ctrchan, unsigned int diochan)
{
    unsigned int index;
    int errcode;
    if (board >= S826_MAX_BOARDS)                                                           // Abort if invalid board number.
...
S826_API int S826_CC S826_CounterExtInRoutingRead(unsigned int board, unsigned int ctrchan, unsigned int *diochan)
{
    unsigned int index;
    int errcode;
...
S826_API int S826_CC S826_CounterModeRead(unsigned int board, unsigned int chan, unsigned int *mode)
{
...
static int BoardInfoRead(unsigned int index, unsigned int *bdnum, unsigned int *bdrev, unsigned int *fpgarev)
{
    unsigned int config;
    unsigned int rev;
                                                                        // Read info registers:
...
S826_API int S826_CC S826_SystemOpen(void)
{
    ...
    int     nboards;
    int     errcode;
    unsigned int    board;              // logical board number (set via dip switches on board)
    unsigned int    garev;
    unsigned int    bdrev;              // ignored
...
S826_API int S826_CC S826_SystemOpen(int index, unsigned int *brd_idx)
{
    int     boardflags = 0;
    int     errcode;
    unsigned int    board;              // logical board number (set via dip switches on board)
...
S826_API int S826_CC S826_SafeControlRead(unsigned int board, unsigned int *settings)
{
    ...
    if (board >= S826_MAX_BOARDS)                                                       // Abort if invalid board number.
        return S826_ERR_BOARD;
    else if ((index = bdindex[board]) == (unsigned int)VACANT_BOARDINDEX)                       // Map board to index number; abort if board closed.
        return S826_ERR_BOARDCLOSED;
...
S826_API int S826_CC S826_SafeWrenWrite(unsigned int board, unsigned int data)
{
    unsigned int index;
    if (board >= S826_MAX_BOARDS)                                                       // Abort if invalid board number.
        return S826_ERR_BOARD;
...
{
    unsigned int index = bdindex[board];
    if (!lcs)
        return -1;
